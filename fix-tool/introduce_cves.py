"""Create 5 branches, each introducing ONE vulnerable TS CVE, then push each.

Each branch adds a minimal vulnerable-ts-app-<pkg>/ with:
...
fix-tool run can remediate each into its own follow-up PR.

Usage: python fix-tool/introduce_cves.py
"""
...
def process(cve: CVEDef, idx: int, total: int) -> str:
    ...
    compare = f"https://github.com/akhil-2009it/auto-remidation-cve/compare/main...{branch}?expand=1"
    print(f"[ok] branch pushed: {branch}")
    print(f"     PR compare URL: {compare}")
    return compare
...
def main() -> None:
    ...
    for i, cve in enumerate(CVES, 1):
        url = process(cve, i, len(CVES))
        urls.append((cve.cve, url))

    run(["git", "checkout", "main"])
    print(f"\n{'=' * 60}")
    print(f"SUMMARY: {len(urls)} branches pushed. Open PRs manually:")
    print(f"{'=' * 60}")
    for cve, url in urls:
        print(f"  {cve:20s}  {url}")
